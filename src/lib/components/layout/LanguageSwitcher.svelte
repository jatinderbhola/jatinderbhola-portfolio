...
<div class="language-switcher relative">
	<button
		type="button"
		class="inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none"
		on:click={toggleMenu}
	...
	{#if isOpen}
		...
		>
			{#each Object.entries(languages) as [code, lang]}
				<button
					class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 {code ===
					$currentLanguage
